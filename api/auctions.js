            ...
            try{
              ...
              if(withFrom.length){
                ...
                if(g && g.from){
                  ...
                  // Год за верхом самого свежего известного поколения. Справочник
                  // часто отстаёт (у Lexus NX «II» помечен 2021–2022, хотя кузов
                  // AL20 идёт до 2025+), поэтому НЕ дробим в микро-диапазон
                  // [верх+1…] — данных там мало, а часто это тот же кузов. Считаем,
                  // что текущее поколение продолжается: от его начала до сегодня.
                  // Similarity-взвешивание по году само придавит далёкие годы.
                  genFrom = newest.from; genTo = curYear;
                }
              }
