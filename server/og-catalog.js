...
const LOC = {
  ...
  ro:{fuel:{4:"Benzină", 1:"Diesel", 3:"Hibrid", 5:"Hibrid plug-in", 2:"Electric"}, tab:{soon:"Azi și mâine", buy_now:"Cumpără acum", archived:"Arhiva vânzărilor"}, sale:{timed:"Timed", no_reserve:"Fără rezervă", on_approval:"În aprobare"},
    from:y => `din ${y}`, to:y => `până în ${y}`, budget:n => `până la $${n} la cheie`, noBan:"fără interdicție la export", canada:"Canada", catalog:"Catalog licitații", kicker:"CATALOG LICITAȚII COPART ȘI IAAI", sub:"Foto · istoric după VIN · preț la cheie", lots:["lot", "loturi", "de loturi"], pluralFn:"ro", onAuctions:"la licitații", auctionsCatalog:"catalog licitații",
...
function plural(n, a, b, c, lang){
  ...
  if(lang === "en") return n === 1 ? a : b;
  const m = n % 100, d = n % 10; return m >= 11 && m <= 14 ? c : d === 1 ? a : d >= 2 && d <= 4 ? b : c;
}
