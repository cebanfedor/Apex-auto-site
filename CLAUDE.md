...
- `action=comps` (`api/auctions.js`): оценка по РЕАЛЬНЫМ проданным лотам из
  ...
  диапазон поколения держится жёстким гейтом до последнего резерва. Первый тир с ≥4
  сопоставимыми. Ведущее число — **диапазон p25–p75** (как DreamBid), медиана вторична.
  Состояние (run&drive) — сильнейший фактор salvage-цены.
