...
- DESC-сортировки без `nullslast` (иначе btree-индекс не используется → таймаут → live 10с); NULL/заглушки отсекаются фильтрами:
  год > 1900, пробег ≤ 1 млн миль (пробег 0 ОСТАВЛЯЕМ — это и реальные новые авто, решение Федора), Buy Now ≥ $300. Смена логики выдачи → бамп `SEARCH_CACHE_VER`.
